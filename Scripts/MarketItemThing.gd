...
var changeAmount=0;
signal item_bought(item_name: String,cost: String,item_description: String,item_pic: String,item_recipe)
...
func _on_buy_pressed():
	if(currentStock>0):
		...
		item_bought.emit($"Ingredient Title".text,$"Cost Num".text,get_meta("IngredientDescription"),get_meta("PicPath"),get_meta("RecipeList"))
